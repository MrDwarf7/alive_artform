...
void displacePoint(const int s, const int j, const float direction,
                   const int randSeed) {
  if (j >= 0 && j < N_POINTS) {
...
void strumOnRadius(const int mouseY, const int s, const float direction,
                   const int randSeed) {
  const int i_center =
      static_cast<int>(mouseY / static_cast<float>(HEIGHT) * N_POINTS);
...
void strumBasic(const int mouseY, const int s, const float direction,
                const int randSeed) {
  const int i =
      static_cast<int>(mouseY / static_cast<float>(HEIGHT) * N_POINTS);
  ...
  for (int s = 0; s < NUM_STRINGS; s++) {
    ...
    if (i >= 0 && i < N_POINTS) { // bounds check
      const float currentX =
          xBase + strings[s][i] * MOUSE_STRENGTH_DISPLACEMENT;
