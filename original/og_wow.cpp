...
void feed_keys(const SDL_Event &event, bool &quit, float &forceRadius, bool &isClicking, int &clickX, int &clickY, SDL_Renderer &renderer) {
    // Handling KEYS
...
float time_delta(Uint32 *lastTime) {
...
int main(int argc, char *argv[]) {
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&event)) {
            feed_keys(event, quit, forceRadius, isClicking, clickX, clickY, *renderer);
        }
